...
test('a set of one is already as small as it goes', () => {
  for (const n of [1, 2, 3, 4]) {
    const ta = new TA(ring, n);
    const spec = parseHsl(SPECIALS.zero.spec(n), n);
    const { root } = ta.fromVectors(spec.vectors.map((v) => toVector(v, ring)));
    assert.equal(reduce(ta, root), root, `${n} qubits: interning had already done it`);
  }
});
...
test('transitions that agree on neither side are left alone', () => {
  ...
  // and the case that *does* merge, beside it, so the pair reads as a contrast
  const shared = ta.state(0, [[A, C], [B, C]]);
