...
export const GATES = {
  id: { matrix: identity(2), label: 'I', doc: 'does nothing' },
  x: { matrix: X, label: 'X', doc: 'bit flip' },
  y: { matrix: Y, label: 'Y', doc: 'bit flip with a phase' },
  z: { matrix: ZZ, label: 'Z', doc: 'flips the sign of |1>' },
  h: { matrix: H, label: 'H', doc: 'Hadamard: |0> becomes (|0>+|1>)/√2' },
  s: { matrix: S, label: 'S', doc: 'multiplies |1> by i' },
  sdg: { matrix: SDG, label: 'S†', doc: 'inverse of s' },
  t: { matrix: T, label: 'T', doc: 'multiplies |1> by ω' },
  tdg: { matrix: TDG, label: 'T†', doc: 'inverse of t' },
  sx: { matrix: SX, label: '√X', doc: 'square root of X' },
  sxdg: { matrix: SXDG, label: '√X†', doc: 'inverse of sx' },
  cx: { matrix: controlled(X), label: 'CX', controls: 1, doc: 'flips the target when the control is 1' },
  cy: { matrix: controlled(Y), label: 'CY', controls: 1, doc: 'applies Y to the target when the control is 1' },
  cz: { matrix: controlled(ZZ), label: 'CZ', controls: 1, doc: 'flips the sign of |11>' },
  ch: { matrix: controlled(H), label: 'CH', controls: 1, doc: 'applies H to the target when the control is 1' },
  cs: { matrix: controlled(S), label: 'CS', controls: 1, doc: 'applies S to the target when the control is 1' },
  csdg: { matrix: controlled(SDG), label: 'CS†', controls: 1, doc: 'applies S† to the target when the control is 1' },
  csx: { matrix: controlled(SX), label: 'C√X', controls: 1, doc: 'applies √X to the target when the control is 1' },
  ct: { matrix: controlled(T), label: 'CT', controls: 1, doc: 'applies T to the target when the control is 1' },
  ctdg: { matrix: controlled(TDG), label: 'CT†', controls: 1, doc: 'applies T† to the target when the control is 1' },
  swap: { matrix: SWAP, label: 'SWAP', swap: true, doc: 'exchanges two qubits' },
  iswap: { matrix: ISWAP, label: 'iSWAP', swap: true, doc: 'exchanges two qubits and multiplies the swapped amplitudes by i' },
  ccx: { matrix: controlled(X, 2), label: 'CCX', controls: 2, doc: 'Toffoli: flips the target when both controls are 1' },
  ccz: { matrix: controlled(ZZ, 2), label: 'CCZ', controls: 2, doc: 'flips the sign of |111>' },
  cswap: { matrix: controlled(SWAP), label: 'CSWAP', controls: 1, swap: true, doc: 'Fredkin: exchanges the last two qubits when the control is 1' },
  ...
  c3x: { matrix: controlled(X, 3), label: 'C³X', controls: 3, doc: 'flips the target when all three controls are 1' },
  c3z: { matrix: controlled(ZZ, 3), label: 'C³Z', controls: 3, doc: 'flips the sign of |1111>' },
};
